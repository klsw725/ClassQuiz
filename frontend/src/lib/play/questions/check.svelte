	...
	<div class="grid grid-rows-2 grid-flow-col auto-cols-auto gap-2 w-full p-4 h-full">
		{#each answers as answer, i}
			...
			>
				{#if game_mode === 'kahoot'}
					{#if answer.emoji}
						<span class="m-auto text-6xl leading-none" aria-label={$t('admin_page.answer_emoji')}
							>{answer.emoji}</span
						>
					{:else}
						<img class="h-2/3 inline-block m-auto" alt={$t('admin_page.answer_icon')} src={kahoot_icons[i]} />
					{/if}
				{:else}
